...
class ApiService {
  ...
  static async getConversationHistory(request: GetHistoryRequest): Promise<GetHistoryResponse> {
    ...
    try {
      const response = await fetchWithTimeout(`${API_BASE_URL}/api/v1/chat/history`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  /**
   * Send a message with automatic session management
   * This endpoint handles session creation and expiration automatically
   */
  static async sendMessage(request: MessageRequest): Promise<QueryResponse> {
    ConnectionLogger.log('request', `Sending message with automatic session management: ${JSON.stringify(request)}`);

    try {
      const response = await fetchWithTimeout(`${API_BASE_URL}/api/v1/chat/message`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const result = await response.json();
      ConnectionLogger.log('response', `Message response received`);
      return result;
    } catch (error: any) {
